...
class UpdateSchedule extends Command
{
    ...
    public function runParseModel(League $model){
        ...
        foreach ($fixtures->fixtures as $fixture) {
            ...
            $matchesHomeHTH = [];
            preg_match("/\d*$/", $fixture->_links->competition->href, $matchesHomeHTH);
            $collectionsHTH = League::where('league_parse_id', $matchesHomeHTH[0])->first();
            $collectionsHTH1 = Schedule::where([
                'league_id' => $collectionsHTH->id,
                'status' => '1'
                ])->get();
            foreach ($collectionsHTH1 as $colle) {
                $HTH = HeadToHead::where([
                    'schedule_id' => $colle->id,
                    'status' => '0'
                    ])->get();
                    foreach ($HTH as $a) {
                        $deleteHTH = HeadToHead::find($a->id)->delete();
                    }
                }

            $games = Game::where('status', 0)->get();
            foreach ($games as $game) {
                $Leagues = UserInGame::where([
                    'game_id' => $game->id,
                    'status' => 0
                ])->get();
                foreach ($Leagues as $League) {
                    $delete = Game::find($League->game_id)->delete();
                }
            }
