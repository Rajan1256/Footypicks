...
class UpdateOneLPay extends Command {
     ...
     * @var string
     */
    protected $signature = 'parse_v2:one:lg:u';
    ...
    public function handle() {
        ...
        //$model = League::whereIn('parse_id_v2',[2021,2015,2002,2014,2019,2001,2146])->get();
        // $model = League::whereIn('parse_id_v2',[2021,2015,2002,2014,2019,2001,2146])->get();
        ...
        foreach ($model as $data) {
            ...
            $to = date('Y-m-d', strtotime("+5 month"));
            ...
            // $url = 'http://api.football-data.org/v2/competitions/'.$data->parse_id_v2.'/matches?dateFrom=2019-01-30&dateTo=2019-03-13';
                ...
                foreach ($ScheduleJson->matches as $el) {
                    ...
                    if ($el->matchday != "") {
                        ...
                        if ($el->matchday == $data->current_matchday + 1) {
                            $homeModel = Team::query()->where('parse_id_v2', (int) $el->homeTeam->id)->
                                            where('league_id', $data->id)->first();

                            $awayModel = Team::query()->where('parse_id_v2', (int) $el->awayTeam->id)->
                                            where('league_id', $data->id)->first();
                ...
                foreach ($ScheduleJson->matches as $el) {
                    ...
                    if ($el->matchday == $data->current_matchday) {
                        ...
                        $homeModel = Team::query()->where('parse_id_v2', (int) $el->homeTeam->id)->
                            ...
                            $chunks = preg_split('/(T|Z)/', $el->utcDate, -1, PREG_SPLIT_NO_EMPTY);
                            $datetimeConvert3 = $chunks[0] . ' ' . $chunks[1];
